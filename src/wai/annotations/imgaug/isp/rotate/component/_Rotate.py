...
class Rotate(
    ...
    threshold = TypedOption(
        "-T", "--threshold",
        type=float,
        help="the threshold to use for Random.rand(): if equal or above, rotation gets applied; range: 0-1; default: 0 (= always)"
    )

    from_degree = TypedOption(
        "-f", "--from-degree",
        type=float,
        help="the start degree to use for rotating the images"
    )

    to_degree = TypedOption(
        "-t", "--to-degree",
        type=float,
        help="the end degree to use for rotating the images"
    )
    ...
    def process_element(
        ...
        polys_aug = None
        if bbs is not None:
            image_aug, bbs_aug = self._seq(image=image, bounding_boxes=bbs)
        elif polys is not None:
            image_aug, polys_aug = self._seq(image=image, polygons=polys)
        else:
            image_aug = self._seq(image=image)
